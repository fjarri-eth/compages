...
def test_unstructure_as_union():
    unstructurer = Unstructurer(
        handlers={Union: unstructure_as_union, int: unstructure_as_int, str: unstructure_as_str}
    )
    assert unstructurer.unstructure_as(Union[int, str], "a") == "a"
    assert unstructurer.unstructure_as(Union[int, str], 1) == 1

    with pytest.raises(UnstructuringError) as exc:
        unstructurer.unstructure_as(Union[int, str], 1.2)
    expected = UnstructuringError(
        r"Cannot unstructure as typing\.Union\[int, str\]",
        [
...
def test_unstructure_as_tuple():
    ...
    expected = UnstructuringError(
        r"Cannot unstructure as typing\.Tuple\[int, str\]",
...
def test_unstructure_as_list():
    ...
    expected = UnstructuringError(
        r"Cannot unstructure as typing\.List\[int\]",
...
def test_unstructure_as_dict():
    ...
    expected = UnstructuringError(
        r"Cannot unstructure as typing\.Dict\[int, str\]",
